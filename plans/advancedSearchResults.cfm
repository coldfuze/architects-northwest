...
<!--- Change the type of plans searched (if applicable) --->
	<cfif IsDefined("URL.so")>
		<cfscript>
            ...
            Session.PlanList = Session.AdvancedSearchPlans;
        </cfscript>
    </cfif>
...
<!--- Convert Session to Local Variables --->
	<cfscript>
        VARIABLES.AdvancedSearchPlans = Session.AdvancedSearchPlans;
        ...
        if (ListLen(VARIABLES.AdvancedSearchPlans) LT VARIABLES.lastPlan) {
            VARIABLES.lastPlan = ListLen(VARIABLES.AdvancedSearchPlans);	
        }
        pdLen = ListLen(VARIABLES.AdvancedSearchPlans);
...
<cfscript>
	pai = StructNew();
	pai.totalrecords = ListLen(VARIABLES.AdvancedSearchPlans);
	...
	if (ListLen(VARIABLES.AdvancedSearchPlans) GT 0) {
		VARIABLES.pic = StructNew();
		VARIABLES.pic.Plans = VARIABLES.AdvancedSearchPlans;
	...
	if (CGI.HTTP_HOST CONTAINS "CornerStone") {
		VARIABLES.pageTitleBar = "Advanced Plan Search Results - Stock Home Plans for Every Style - CornerStone Designs";	
	}
	else {
		VARIABLES.pageTitleBar = "Advanced Plan Search Plan Results - Stock Home Plans for Every Style - Your Family Architect - Architects Northwest";		
	}
...
<!--- Get Quick Search Business Logic --->
    ...
    <cfmodule template="#Application.CTPath#planLayout.cfm" title="#VARIABLES.pageTitleBar#" recentlyViewedPlans="#VARIABLES.RecentlyViewedPlans#" minHeight="#VARIABLES.minHeight#" cannedSearch="1">
            <div id="centerColumn">
            	<div id="titleBarPS">
                	ADVANCED PLAN SEARCH RESULTS
                </div>
                <cfif ListLen(VARIABLES.AdvancedSearchPlans) EQ 0>
                    <div style="text-align:center;color:red;margin:10px 0;">
                    There are no plans that match your search criteria.
                    </div>
                <cfelse>
                <div class="planSearchResultsNavigation">
                	<div class="planSearchResultCount">
                    	Results: <cfoutput>#VARIABLES.FirstPlan#-#VARIABLES.lastPlan# of #ListLen(VARIABLES.AdvancedSearchPlans)#</cfoutput>
                    </div>
