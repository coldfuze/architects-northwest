<cfcomponent extends="PlanGateway">
    ...
    <cffunction name="getPulldownFromPlanNumber" access="remote" returntype="string" hint="This method returns a value list to be used in an auto-suggest pulldown of plans">
        <cfscript>
			var gi="";
			var plans = "";
		</cfscript>
    ...
    <cffunction name="getPlanTitlePicklistforQuickSearch" access="remote" returntype="query" hint="This method returns a query for use in the Quick Search plan title picklist">
    	<cfscript>
			var gi="";
			var plans = [];
		</cfscript>
    ...
    <cffunction name="doesPlanHaveMedia" access="public" returntype="struct" hont="This method returns a boolean value as to whether a plan has video or audio, or a 360 degree animation">
        ...
        <cfquery datasource="#Application.DSN#" username="#Application.LimitedUser#" password="#Application.LimitedPassword#" name="mediaInfo">
        	SELECT 	Video, Animation, Animation360, YouTube
            FROM	[Plan]
            WHERE	PlanId = <cfqueryparam cfsqltype="cf_sql_integer" value="#ARGUMENTS.PlanId#">
        </cfquery>
        ...
        <cfscript>			
			...
			if (LEN(TRIM(mediaInfo.YouTube)) GT 0) {
				mi.hasYouTube = 1;	
				mi.YouTube = mediaInfo.YouTube;
				mi.hasMedia = 1;
			}
		</cfscript>
    ...
    <cffunction name="getSimilarPlans" access="public" returntype="query" hint="This method returns related plans for a plan">
    	<cfargument name="PlanId" required="yes" type="numeric" hint="This argument expects a numeric PlanId">
        <cfscript>
			var getRelatedPlanList = "";
			var RelatedPlanList = 0;
			var getRelatedPlanInfo = "";
		</cfscript>
        ...
        <cfscript>
			if (getRelatedPlanList.recordCount GT 0) {
				RelatedPlanList = ValueList(getRelatedPlanList.relatedPlanId);
			}
		</cfscript>
        ...
        <cfquery datasource="#Application.DSN#" username="#Application.LimitedUser#" password="#Application.LimitedPassword#" name="getRelatedPlanInfo">
            SELECT	b.PlanId As RelatedPlanId,
             ...
             WHERE	a.isPlanThumbnail = 1
                    AND a.PlanId IN (<cfqueryparam cfsqltype="cf_sql_integer" list="yes" value="#RelatedPlanList#">)
                    AND b.DisplayPlan = 1
        </cfquery>
    ...
    <cffunction name="getRecentlyViewedPlans" access="public" returntype="query" hint="This method returns recently viewed plans">
        ...
        <cfscript>
			if (ArrayLen(ARGUMENTS.Plans) GT 0) {
				planList = ArrayToList(ARGUMENTS.Plans);
			}
		</cfscript>
        ...
        <cfquery datasource="#Application.DSN#" username="#Application.LimitedUser#" password="#Application.LimitedPassword#" name="gi">
            SELECT	b.PlanId,
             ...
             FROM	PlanImage a
                    ...
                    LEFT JOIN FullBathroom d ON b.FullBathroomId = d.FullBathroomId
             WHERE	a.isPlanThumbnail = 1
                    AND a.PlanId IN (<cfqueryparam cfsqltype="cf_sql_integer" list="yes" value="#planList#">)
                    AND b.DisplayPlan = 1
        </cfquery>
    ...
    <cffunction name="getFeaturedPlanInfoForSearchSubGrouping" returntype="query" access="public" hint="This method returns a single plan to display for a particular Search Sub Grouping">
	 ...
	 <cffunction name="getFeaturedPlanInfoForGarageAndAccessory" returntype="query" access="public" hint="This method returns a single plan to display for Garage and Accessory plans">        
        <cfquery datasource="#Application.DSN#" username="#Application.LimitedUser#" password="#Application.LimitedPassword#" name="gi" maxrows="1">
        	SELECT	a.PlanId,
            		a.PlanNumber,
                    CASE WHEN LEN(a.PlanShortTitle) > 0 THEN UPPER(a.PlanShortTitle)
            ...
            FROM	[Plan] a
                    ...
                    LEFT JOIN Bedroom d ON a.BedroomId = d.BedroomId 
                    LEFT JOIN PlanImage e ON a.PlanId = e.PlanId
            WHERE   (a.DisplayPlan = 1) AND (e.isPlanThumbnail = 1) AND PlanTypeId = 7
			ORDER BY NEWID()
        </cfquery>
    ...
    <cffunction name="deletePlan" access="public" returntype="numeric" hint="This method deletes a plan and its related content">
	    <cfargument name="PlanId" type="numeric" required="yes" hint="This argument expects a numeric PlanId">
        <cfscript>
			var planInfo = Application.PlanService.getById(ARGUMENTS.PlanId);
			var deletePlanInfo = Application.PlanService.deleteById(ARGUMENTS.PlanId);
			var deletePlanImages = Application.planImageService.deleteByPlanId(ARGUMENTS.PlanId);
			var deleteGarageMappings = Application.PlanToGarageTypeService.deleteGarageTypesForPlan(ARGUMENTS.PlanId);
			var deleteSearchSubGroupingsForPlan = Application.PlanToSearchSubGroupingService.deleteSearchSubGroupingsForPlan(ARGUMENTS.PlanId);
			var deleteRelatedPlansForAPlan = Application.RelatedPlanService.deleteRelatedPlansForAPlan(ARGUMENTS.PlanId);
			var tempImagePath = '/assets/planImages/' & ARGUMENTS.PlanId;
			var imagePath = ExpandPath(tempImagePath);
			var tempFloorPlanPath = '/assets/floorPlans/' & ARGUMENTS.PlanId;
			var floorPlanPath = ExpandPath(tempFloorPlanPath);
			var tempAnimation360Path = '/assets/animation360/' & ARGUMENTS.PlanId;
			var animation360Path = ExpandPath(tempAnimation360Path);
			var animationPath = "";
			var animationFile = "";
			var videoPath = "";
			var videoFile = "";
			var success = 1;
			if (LEN(TRIM(planInfo.Animation)) GT 0) {
				animationPath = '/assets/animations/' & planInfo.Animation;
				animationFile = ExpandPath(animationPath);
			}
			if (LEN(TRIM(planInfo.Video)) GT 0) {
				videoPath = '/assets/videos/' & planInfo.Video;
				videoFile = ExpandPath(videoPath);
			}
		</cfscript>
        
        <cfif DirectoryExists(imagePath)>
            <cfdirectory action="delete" directory="#imagePath#" recurse="yes">
        </cfif>
        
        <cfif DirectoryExists(floorPlanPath)>
            <cfdirectory action="delete" directory="#floorPlanPath#" recurse="yes">
        </cfif>
        
        <cfif DirectoryExists(animation360Path)>
            <cfdirectory action="delete" directory="#animation360Path#" recurse="yes">
        </cfif>
        
        <cfif LEN(TRIM(animationFile)) GT 0>
			<cfif FileExists(animationFile)>
                <cffile action="delete" file="#animationFile#">
            </cfif>
        </cfif>
        
        <cfif LEN(TRIM(videoFile)) GT 0>
			<cfif FileExists(videoFile)>
                <cffile action="delete" file="#videoFile#">
            </cfif>
        </cfif>
    ...
    <cffunction name="deleteFloorPlans" access="public" roles="admin" returntype="void" hint="This method deletes selected floor plans for a plan">
    	<cfargument name="fpi" type="struct" required="yes" hint="This argument expects a structure of Floor Plan Information">
        <cfscript>
			VARIABLES.PlanInfo = Application.PlanService.getById(ARGUMENTS.fpi.PlanId);
			VARIABLES.floorPlanBasePath = 'D:\home\architectsnw.com\wwwroot\assets\floorplans\' & ARGUMENTS.fpi.PlanId;
        ...
        <cfif ARGUMENTS.fpi.delMainFloorPlan EQ 1>
			<cfif LEN(TRIM(VARIABLES.PlanInfo.MainFloorPlan GT 0))>
                ...
                <cfif FileExists(VARIABLES.delFullImage)>
                	<cffile action="delete" file="#VARIABLES.delFullImage#">
                </cfif>
                <cfif FileExists(VARIABLES.delFullReverseImage)>
                	<cffile action="delete" file="#VARIABLES.delFullReverseImage#">
                </cfif>
                <cfif FileExists(VARIABLES.delNormalImage)>
                	<cffile action="delete" file="#VARIABLES.delNormalImage#">
                </cfif>
                <cfif FileExists(VARIABLES.delNormalReverseImage)>
                	<cffile action="delete" file="#VARIABLES.delNormalReverseImage#">
                </cfif>
                <cfquery datasource="#Application.DSN#" username="#Application.UserName#" password="#Application.Password#" name="dp">
                	UPDATE [Plan]
                    SET MainFloorPlan = NULL
                    WHERE PlanId = <cfqueryparam cfsqltype="cf_sql_integer" value="#ARGUMENTS.fpi.PlanId#">
                </cfquery>
			</cfif>
        </cfif>
        <cfif ARGUMENTS.fpi.delUpperFloorPlan EQ 1>
			<cfif LEN(TRIM(VARIABLES.PlanInfo.UpperFloorPlan GT 0))>
                ...
                <cfquery datasource="#Application.DSN#" username="#Application.UserName#" password="#Application.Password#" name="dp">
                	UPDATE [Plan]
                    SET UpperFloorPlan = NULL
                    WHERE PlanId = <cfqueryparam cfsqltype="cf_sql_integer" value="#ARGUMENTS.fpi.PlanId#">
                </cfquery>
			</cfif>
        </cfif>
        <cfif ARGUMENTS.fpi.delLowerFloorPlan EQ 1>
			<cfif LEN(TRIM(VARIABLES.PlanInfo.LowerFloorPlan GT 0))>
                ...
                <cfquery datasource="#Application.DSN#" username="#Application.UserName#" password="#Application.Password#" name="dp">
                	UPDATE [Plan]
                    SET LowerFloorPlan = NULL
                    WHERE PlanId = <cfqueryparam cfsqltype="cf_sql_integer" value="#ARGUMENTS.fpi.PlanId#">
                </cfquery>
			</cfif>
        </cfif>
        <cfif ARGUMENTS.fpi.delThirdFloorPlan EQ 1>
			<cfif LEN(TRIM(VARIABLES.PlanInfo.ThirdFloorPlan GT 0))>
                ...
                <cfquery datasource="#Application.DSN#" username="#Application.UserName#" password="#Application.Password#" name="dp">
                	UPDATE [Plan]
                    SET ThirdFloorPlan = NULL
                    WHERE PlanId = <cfqueryparam cfsqltype="cf_sql_integer" value="#ARGUMENTS.fpi.PlanId#">
                </cfquery>
			</cfif>
        </cfif>
    </cffunction>
    <cffunction name="getInactivePlans" access="public" returntype="query" hint="This method returns a query of inactive plans">
    	<cfscript>
			var gi = "";
		</cfscript>
        
        <cfquery datasource="#Application.DSN#" username="#Application.LimitedUser#" password="#Application.LimitedPassword#" name="gi">
        	SELECT PlanId, PlanNumber, PlanTitle
            FROM [Plan]
			WHERE DisplayPlan = 0
            ORDER BY PlanNumber
        </cfquery>
